import java.util.*;

public class AES {
    private static final int Nb = 4; 
    private static final int Nk = 4; 
    private static final int Nr = 10; 

    private static final int[] S_BOX = {
        
    };


    private static final int[] R_CON = {
        0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
    };

    private static byte[][] state;
    private static byte[][] roundKeys;

    public static void main(String[] args) {
        Scanner input = new Scanner(System.in);
        System.out.println("Enter Plain Text: ");
        String plaintext = input.nextLine();
        System.out.println("Enter Key: ");
        String key = input.nextLine();

        byte[] plainBytes = plaintext.getBytes();
        byte[] keyBytes = key.getBytes();

        roundKeys = keyExpansion(keyBytes);

        state = new byte[4][Nb];
        for (int i = 0; i < plainBytes.length; i++) {
            state[i % 4][i / 4] = plainBytes[i];
        }
        addRoundKey(0);
        for (int round = 1; round < Nr; round++) {
            subBytes();
            shiftRows();
            mixColumns();
            addRoundKey(round);
        }
        subBytes();
        shiftRows();
        addRoundKey(Nr);
         
        System.out.println("Ciphertext:");
        for (int col = 0; col < Nb; col++) {
            for (int row = 0; row < 4; row++) {
                System.out.printf("%02X ", state[row][col]);
            }
        }
    }

    private static byte[][] keyExpansion(byte[] key) {
        byte[][] w = new byte[Nb * (Nr + 1)][4];
        int i = 0;

        while (i < Nk) {
            w[i][0] = key[4 * i];
            w[i][1] = key[4 * i + 1];
            w[i][2] = key[4 * i + 2];
            w[i][3] = key[4 * i + 3];
            i++;
        }

        i = Nk;
        while (i < Nb * (Nr + 1)) {
            byte[] temp = w[i - 1].clone();
            if (i % Nk == 0) {
                temp = subWord(rotWord(temp));
                temp[0] ^= R_CON[(i / Nk) - 1];
            }
            for (int t = 0; t < 4; t++) {
                w[i][t] = (byte) (w[i - Nk][t] ^ temp[t]);
            }
            i++;
        }
        return w;
    }

    private static byte[] rotWord(byte[] word) {
        byte[] result = new byte[4];
        result[0] = word[1];
        result[1] = word[2];
        result[2] = word[3];
        result[3] = word[0];
        return result;
    }

    private static byte[] subWord(byte[] word) {
        byte[] result = new byte[4];
        for (int i = 0; i < 4; i++) {
            result[i] = (byte) S_BOX[word[i] & 0xFF];
        }
        return result;
    }

    private static void subBytes() {
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < Nb; col++) {
                state[row][col] = (byte) S_BOX[state[row][col] & 0xFF];
            }
        }
    }

    private static void shiftRows() {
        for (int row = 1; row < 4; row++) {
            byte[] temp = new byte[Nb];
            for (int col = 0; col < Nb; col++) {
                temp[col] = state[row][(col + row) % Nb];
            }
            state[row] = temp;
        }
    }

private static void mixColumns() {
    for (int col = 0; col < Nb; col++) {
        byte[] temp = new byte[4];
        temp[0] = (byte) (galoisMult(state[0][col], 2) ^ galoisMult(state[1][col], 3) ^ galoisMult(state[2][col], 1) ^ galoisMult(state[3][col], 1));
        temp[1] = (byte) (galoisMult(state[0][col], 1) ^ galoisMult(state[1][col], 2) ^ galoisMult(state[2][col], 3) ^ galoisMult(state[3][col], 1));
        temp[2] = (byte) (galoisMult(state[0][col], 1) ^ galoisMult(state[1][col], 1) ^ galoisMult(state[2][col], 2) ^ galoisMult(state[3][col], 3));
        temp[3] = (byte) (galoisMult(state[0][col], 3) ^ galoisMult(state[1][col], 1) ^ galoisMult(state[2][col], 1) ^ galoisMult(state[3][col], 2));

        state[0][col] = temp[0];
        state[1][col] = temp[1];
        state[2][col] = temp[2];
        state[3][col] = temp[3];
    }
}

private static byte galoisMult(byte a, int b) {
    byte result = 0;
    byte highBitMask = (byte) 0x80;
    byte highBit = 0;
    byte modulo = 0x1B;

    for (int i = 0; i < 8; i++) {
        if ((b & 1) == 1) {
            result ^= a;
        }
        highBit = (byte) (a & highBitMask);
        a <<= 1;
        if (highBit != 0) {
            a ^= modulo;
        }
        b >>= 1;
    }
    return result;
}

    private static void addRoundKey(int round) {
        for (int col = 0; col < Nb; col++) {
            for (int row = 0; row < 4; row++) {
                state[row][col] ^= roundKeys[round * Nb + col][row];
            }
        }
    }

}
